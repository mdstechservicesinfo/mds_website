...
function Packages() {
  ...
  // Re-run reveal on tab switch so new cards animate in
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting) e.target.classList.add('visible');
    ...
    <PageTransition>
      <div style={{ background: "#040610", minHeight: "100vh", color: "#fff", overflowX: "hidden" }}>
        <style>{`
          ...
          .tab-btn:hover { color: rgba(255,255,255,0.8); border-color: rgba(255,255,255,0.12); }
          ...
          .packages-grid {
            ...
            gap: 24px;
            align-items: start;
            animation: tabFadeIn 0.4s cubic-bezier(0.4,0,0.2,1) both;
          }
          .pkg-card {
            padding: 40px 36px;
            background: rgba(255,255,255,0.025);
          ...
          .pkg-card::after {
            content: '';
            position: absolute; inset: 0;
            background: radial-gradient(circle at 50% 0%, var(--pkg-glow, rgba(37,99,235,0.08)) 0%, transparent 60%);
            opacity: 0;
          ...
          .pkg-feature:hover { color: rgba(255,255,255,0.95); }
          .pkg-check {
        ...
        <Helmet>
          ...
          <meta name="description" content="Choose from our Website, E-Commerce, Booking, School, or POS packages. Professional software solutions tailored to your business needs." />
          <meta property="og:title" content="Packages | MDS Software Development Services" />
          <meta property="og:url" content="https://mdstechservices-ph.online/packages" />
        </Helmet>
        ...
        <section className="page-hero">
          ...
          <p className="page-desc">
            From simple websites to full enterprise systems — choose the product and tier that fits your business perfectly.
          </p>
        </section>
        ...
        <div className="tabs-wrap">
          ...
          <p className="tab-category-desc reveal reveal-d1">{currentCategory.desc}</p>
        </div>
          ...
          <div className="packages-grid" key={activeCategory}>
            {currentCategory.packages.map((pkg, i) => (
              ...
              >
                ...
                <div>
                  <div className="pkg-name" style={{ color: pkg.color }}>{pkg.name} Package</div>
                ...
                <div className="pkg-features">
                  {pkg.features.map((f, j) => (
                    <div className="pkg-feature" key={j}>
                      <div className="pkg-check" style={{ background: `${pkg.color}22`, color: pkg.color }}>✓</div>
                      {f}
                    </div>
                  ))}
                </div>
          ...
          <div className="modal-backdrop" onClick={() => setSelectedPackage(null)}>
            <div className="modal-box" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <div>
                  <div className="modal-title">{selectedPackage} Package Inquiry</div>
                  <div className="modal-subtitle">{selectedCategoryLabel} · We'll get back to you within 24 hours</div>
                  <div className="modal-payment-note">💳 We accept GCash, bank transfer, and check payments</div>
                </div>
              ...
              <div className="modal-body">
                <div className="form-group">
                  ...
                  <input className="form-input" type="text" name="name" placeholder="Juan dela Cruz" value={form.name} onChange={handleChange} />
                </div>
                <div className="form-group">
                  <label>Email Address</label>
                  <input className="form-input" type="email" name="email" placeholder="juan@email.com" value={form.email} onChange={handleChange} />
                </div>
                <div className="form-group">
                  <label>Company Name (Optional)</label>
                  <input className="form-input" type="text" name="company" placeholder="Your Company" value={form.company} onChange={handleChange} />
                </div>
                <div className="form-group">
                  <label>Message</label>
                  <textarea className="form-textarea" name="message" placeholder="Tell us about your project and requirements..." value={form.message} onChange={handleChange} />
                </div>
                <button className="btn-submit" onClick={handleSubmit} disabled={loading}>
                  {loading ? "Sending..." : `Inquire About ${selectedPackage} Package →`}
                </button>
                {status === "success" && <div className="success-msg">✅ Inquiry sent! We'll contact you within 24 hours.</div>}
                {status === "error" && <div className="error-msg">❌ Something went wrong. Please try again.</div>}
                {status === "missing" && <div className="missing-msg">⚠️ Please fill in your name, email and message.</div>}
              </div>
            </div>
          </div>
        )}
